...
export default function QuoteBuilder({ dbRates = [] }: { dbRates?: ServiceRate[] }) {
    ...
    const EXCHANGE_RATE = 1200 // Mock Rate (approx blue dollar or official+taxes), can be env var

    // --- Currency Helper ---
    const formatMoney = (amount: number) => {
        const value = currency === 'ARS' ? amount * EXCHANGE_RATE : amount
            ...
            try {
                ...
                const formatMoney = useCallback((amount: number) => {
                    return new Intl.NumberFormat('en-US', {
                        ...
                        currency: currency,
                        maximumFractionDigits: 0
                    }).format(amount * EXCHANGE_RATES[currency]) // Apply conversion for display
                }, [currency])
                ...
                const handleSaveQuote = async () => {
                    ...
                    setIsSaving(true);
                    try {
                        // Calculate base USD values first
                        const totalMonthlyCost = calculateTotalMonthlyCost()
                        const l2SupportCost = calculateL2SupportCost(totalMonthlyCost)
                        const riskCost = calculateRiskCost(totalMonthlyCost)
                        const totalWithRisk = calculateTotalWithRisk()
                        const discountAmount = totalWithRisk * (state.commercialDiscount / 100)

                        // CONVERTED FINAL TOTAL
                        const finalTotalUSD = totalWithRisk - discountAmount
                        const finalTotalConverted = convert(finalTotalUSD)
                        const exchangeRate = EXCHANGE_RATES[currency]

                        // 1. SAVE TO DB (Store Converted Amount if that is what client sees, or USD? 
                        // Better to store Converted so Dashboard matches PDF. 
                        // We save currency metadata in technicalParameters)

                        const breakdown = {
                            totalMonthlyCost, l2SupportCost, riskCost, totalWithRisk, discountAmount, finalTotal: finalTotalUSD
                        }

                        const result = await saveQuote({
                            ...state,
                            estimatedCost: finalTotalConverted, // Storing CONVERTED value
                        ...
                        const blob = await generatePDFBlob({
                            ...
                            discountAmount,
                            finalTotal: finalTotalUSD, // PDF Generator will handle conversion if we pass params
                            // PASS CURRENCY PARAMS
                            currency: currency,
                            exchangeRate: exchangeRate
                        })
